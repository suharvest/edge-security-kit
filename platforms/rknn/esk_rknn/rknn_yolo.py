...
class RKNNPersonDetector:
    ...
    @property
    def head(self) -> str:
        """Which head layout this model has: ``ultralytics`` or ``zoo``."""
    ...
    def detect(
        ...
        # own answer rather than from configuration means a model swapped
        # underneath this process cannot be silently mis-decoded.
        if len(outputs) == 1:
