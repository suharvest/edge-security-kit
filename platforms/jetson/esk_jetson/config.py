"""Runtime configuration for the Jetson Orin TensorRT detector."""
...
class Config:
    ...
    appsink_queue: int = 3
    # This platform's primary decode path is NVDEC via nvv4l2decoder.
    ...
    # Model. A TensorRT engine is only valid for the exact TensorRT version and
    # GPU it was built on, so this path names a build product of the target
    # device, never something shipped in the repository.
    model: str = "models/yolov8n_fp16.orin.engine"
    input_size: int = 640
