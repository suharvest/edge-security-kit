...
The frame served here is the *active region* of the letterbox canvas: the
decoded image with the grey padding cut back off. NVDEC does hand back a
full-resolution frame, unlike the Rockchip path, but serving it would mean
copying 1280x720x4 into the frame store on every frame to satisfy an endpoint
polled once a second. The served image is genuinely decoded pixels at the
aspect ratio of the source, just smaller (1280x720 arrives as 640x360), which
keeps this platform's preview and snapshot behaviour identical to
``platforms/rknn``'s. The rule canvas works in normalized coordinates, so it is
unaffected, and the snapshot is a real frame rather than a re-render.
"""
