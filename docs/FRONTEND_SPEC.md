...
导航：hub 顶栏四个 tab + 语言切换 + 当前用户名。设备本地调试页无导航，页顶注明"调试模式——生产使用请访问 hub"。
...
录像时间轴、NVR 存储管理、PTZ、电子地图、多租户权限、工单流转、人脸/车牌检索。

**"hub 侧视频墙"曾在这份清单里，现在不在了**（§10）。当时把它列为非目标，防的是
hub 变成 VMS——为每路流维持长连接、按帧率拉取、向每个观看者扇出。§10 的墙不做这件
事：画面由浏览器直连设备的 `live_url`，hub 只出叠框 JSON 和离线格的单帧代理，成本
与观看人数无关。被防的那件事仍然是禁止的（HUB_SPEC §11），措辞收窄到它本身。

## 10. 视频墙（/wall）

一屏看完所有摄像头，接上 HDMI 大屏就是值班室的那块屏。

### 布局

自适应网格 1 / 2 / 4 / 6 / 9 路，加一个"自动"档：自动 = **能装下当前全部流的最小
布局**，所以插上第五路摄像头不会被悄悄藏进分页；超过 9 路封顶 9，并在总览条上写
"还有 N 路未显示"。手动选的档位记在 localStorage，值班员换班回来还是他那块屏。

列数固定、行数推导（6 = 3×2，9 = 3×3），换台电脑打开不会变成 2×3。

### 总览条（不看画面也能读）

`8/9 路在线` + `1 路离线` + `1 路软解码` + `合计 95.0 fps`。这是操作员在看任何一格
之前先读的一行——离线和软解码用色块标出来，因为它们是要动手的两件事。

### 每格

- **实时画面**：浏览器直连该流 status 里的 `live_url`（站点部署了 go2rtc 就是
  WebRTC，没部署就是探测器自带的刷新静帧）。**视频不经 hub**——理由见 HUB_SPEC §11。
- **叠框**：检测器逐帧结果（`GET /api/live`，全屏一次请求）画框 + track_id + 分数，
  加上该流的禁区多边形与越线段。画的是"告警会拿什么去判"，不只是"画面里有人"。
- **超时不画**：detections 超过 4 s 未更新就不再画框。卡住的流留着上一批框，是让人
  信任一格死画面的那种失败。
- **叠框与画面共用同一个盒子**：盒子按 `frame.w/h` 定宽高比、在格子里居中，画面和
  SVG 都填满它。画面按比例内缩、叠框却铺满整格的话，4:3 源在 16:9 格子里每个框都会
  偏掉一个 pad——和 `frame_norm` 在线上防的是同一类错。
- **离线格**：灰底 + 「设备离线 / 显示最近一帧快照」，底图回落到 hub 的单帧代理缓存。
  最后看见的画面比一块黑好，前提是标明它是旧的。**设备在线但流在 reconnecting 也算
  不在线**：那一格再画实时画面，画的是几分钟前的现场。

### 全屏（HDMI）

`F` 进入 / 退出，`Esc` 退出。全屏元素是墙本身，顶栏与页面外壳自然不在里面，不需要
另写一套隐藏逻辑。按键监听挂在 window 上——走到墙前的人没点过任何东西；但输入框、
下拉框获得焦点时不抢键。全屏下隐藏每格的删除按钮：大屏是给人看的，不是给人误触的。

### 置信度滑块

每路一个，范围 0.05–0.95、步长 0.05。步长不做 0.01：0.35 和 0.36 在现场看不出区别，
装作能区分只会让人反复拨而不是做决定。

- 拖动时显示本地值，松手后显示**设备实际生效的值**（`applied.conf_threshold`）。保存
  失败还停在拖到的位置，会让人以为现场变了。
- 三种结果三种呈现：200 提示"已生效：0.45"；409 提示设备给的原因；**504 明说"没有
  回应，是否生效未知"**，并刷新设备列表让操作员看设备自己上报的值。

### 添加 / 移除摄像头

对话框：挂到哪台检测器（只列在线的）、RTSP 地址、名称、流 ID（自动填下一个可用的
`cam-N`）、传输方式。

- **地址里的密码不上屏**：填完在下方回显 `rtsp://admin:***@host/path`。截图、投屏、
  工单里都不该带着摄像头密码走。
- 流 ID 含空格 / `/` / `+` / `#` 一律挡在前端——它是 MQTT 主题的一段。
- 提交后按钮变"正在连接摄像头…"，因为 hub 要等检测器真的把流打开才回 201。
- **504 时对话框不关**：设备没回应，摄像头可能加了也可能没加，关掉等于替操作员断言
  了一个结果。文案直说"刷新设备列表确认后再重试"。

### 验收标准

- 6 路的墙上每格都能看到自己的画面和框；把一台设备断网，≤10 s 该格转灰并写明离线。
- 按 F 进全屏，顶栏消失、9 格铺满；Esc 回来，布局不变。
- 拖一格的置信度并松手，探测器下一帧就用新阈值（设备端 status 上报的值随之改变），
  且 `GET /api/audit` 里有一条带操作人、前后值的记录。
- 在对话框里填一个 RTSP 地址，提交后该摄像头出现在墙上并开始出框，全程不碰配置文件、
  不重启容器。
- 把 hub 的 ack 人为拖到超时（`/mock/control?mode=timeout`），置信度与添加摄像头两处
  都提示"未知"，都没有把自己画成成功。
